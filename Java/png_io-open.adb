-- $Id: png_io-open.adb 94 2020-01-05 17:36:08Z sangwine $
---------------------------------------------------------------------
---------------------------------------------------------------------
-- PNG_IO: Ada 2012 Portable Network Graphics Input/Output Package --
--                                                                 --
--                http://png-io.sourceforge.net/                   --
--                                                                 --
--        Copyright (©) 1999-2020 Dr Stephen J. Sangwine           --
          ...
          for X in 1 .. W loop      -- For each byte to be filtered.
            ...
            declare
              ...
              function Previous return Stream_Element is
                (if X <= Bpp then 0
                             else Data(Pointer - Stream_Element_Offset(Bpp)));
    
              function Prior return Stream_Element is
                (if Y = 1 then 0 else Data(P + Stream_Element_Offset(X)));
    
              function PrevPrior return Stream_Element is
                (if X <= Bpp or Y = 1 then 0
                                      else Data(P + Stream_Element_Offset(X - Bpp)));    
            begin
              case FT is
